...
def get_blog_by_title(title):
    cursor.execute(f'SELECT * FROM blogtable WHERE title="{title}"')
    data = cursor.fetchall()
    return data


def get_blog_by_author(author):
    cursor.execute(f'SELECT * FROM blogtable WHERE author="{author}"')
    data = cursor.fetchall()
    return data


def delete_data(title):
    cursor.execute(f'DELETE FROM blogtable WHERE title="{title}"')
    conexion.commit()
...
def main():
    ...
    if choice == "Home":
        ...
        for i in result:
            ...
            b_post_date = i[3]
            st.markdown(
                title_temp.format(b_title, b_author, b_article, b_post_date),
        ...
        for i in post_result:
            ...
            b_article = i[2]
            b_post_date = i[3]
            # st.text(f"Reading Time:{readingTime(b_article)}")
            st.markdown(
                head_message_temp.format(b_title, b_author, b_post_date),
                unsafe_allow_html=True,
            )
